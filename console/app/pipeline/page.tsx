...
const stageIcons: Record<string, string> = {
  ...
  MARKETMATCHING: "Market Matching",
...
const statusBadge = (status: string) => {
  ...
  if (lower === "healthy" || lower === "completed" || lower === "running")
...
export default function PipelinePage() {
  ...
  const pipelineOrder = [
    "Discovery", "Registry", "Market Matching", "Collectors",
    "Database", "Finalizer", "Repair", "Completed",
  ];
              ...
              <div className="flex flex-col items-center">
                ...
                {i < stages.length - 1 && (
                  <div className={`mt-1 h-12 w-0.5 ${
                    stage.status.toLowerCase() === "healthy" ? "bg-emerald-500/30" : "bg-slate-700"
                  }`} />
                )}
              </div>
              ...
              <Card className="flex-1">
                ...
                <CardContent>
                  <div className="grid grid-cols-4 gap-4 text-sm">
                    ...
                    <div>
                      <span className="block text-xs text-slate-500">Items Processed</span>
                      <span className="text-slate-300">{stage.items_processed}</span>
                    </div>
                    <div>
                      <span className="block text-xs text-slate-500">Errors</span>
                      <span className={stage.errors > 0 ? "text-red-400" : "text-slate-300"}>
                        {stage.errors}
                      </span>
                    </div>
                    <div>
                      ...
                      <span className="text-slate-300">
                        {stage.duration_sec != null ? `${stage.duration_sec.toFixed(1)}s` : "—"}
                      </span>
                    </div>
                  </div>
                </CardContent>
